...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $dob = $_POST["dob"];
    $skills = $_POST["skills"];
    $resumeDir = 'uploads/'; // Specify the directory where you want to store resumes
    $resumePath = $resumeDir . basename($_FILES['resume']['name']);
    $uploadOk = 1;

    // Check file size, file type, etc. if needed
    if (move_uploaded_file($_FILES['resume']['tmp_name'], $resumePath)) {
        //$fileName = $_FILES['resume']['name'];
        $sql = "INSERT INTO seeker (name,email,password,qualification,dob,skills,resume)
                VALUES ('$name', '$email', '$password','$qlf', '$dob', '$skills','$resumePath')";
